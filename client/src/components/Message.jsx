...
const Message = ({ message, setReplyMessage }) => {
  ...
  return (
    ...
    >
      <div
        className={`group relative inline-block max-w-[85%] sm:max-w-[75%] md:max-w-[65%]
 w-fit px-4 py-2 pr-8 rounded-2xl shadow-sm transition ${
   isMe
        ...
        {message.type === "file" && (
          ...
          >
            <div className="flex items-center gap-3">
              {/* PDF Icon */}
              <div className="bg-red-500 text-white font-bold px-2 py-1 rounded text-xs shrink-0">
                PDF
              </div>

              {/* File Info */}
              <div className="flex-1 min-w-0">
                <p className="font-semibold break-words text-sm leading-snug">
                  {message.content}
                </p>
